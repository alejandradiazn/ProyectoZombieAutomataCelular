...
**Proceso cuando están activos:**
1. Si hay células infectadas cercanas y el macrófago tiene más de 40 puntos de energía:
   - Se evalúa aleatoriamente si la fagocitosis tiene éxito (55% de probabilidad)
   - Si tiene éxito: reduce la carga viral en 25 puntos y gasta 30 puntos de energía
   - **Muerte por agotamiento**: Si la energía cae por debajo de 25 o si ha estado activo por más de 15 turnos, el macrófago muere y se convierte en plasma
...
Las plaquetas son vulnerables a la infección viral directa. Si hay 2 o más virus cercanos y la carga viral de la plaqueta supera 40, se convierte en una partícula de virus.
...
El sistema inmune puede destruir virus libres. La probabilidad base de eliminación es del 60%, aumentando según el nivel de anticuerpos (cada 200 puntos de anticuerpos suman proporcionalmente a la probabilidad).

**Condiciones:** Debe haber al menos 2 células inmunes cercanas o el nivel de anticuerpos debe superar 60. Si se elimina, la partícula viral se convierte en plasma.

#### Replicación Viral
Después de estar libre por más de 4 turnos y si hay menos de 3 virus en el área, el virus tiene un 25% de probabilidad de generar una explosión viral, multiplicándose exponencialmente.
...
**Nota crítica:** Esta probabilidad decreciente significa que mientras más tiempo pase una célula infectada, más difícil será salvarla. El rescate temprano es crucial.
...
Las células zombie son extremadamente difíciles de eliminar, requiriendo una respuesta inmune masiva:
...
**Característica clave:** Los zombies representan células senescentes que persisten en el sistema, siendo muy difíciles de eliminar y causando daño continuo.
...
La carga viral de cada célula se actualiza en cada turno considerando:
...
La carga viral siempre se mantiene entre 0 y 100 puntos.

### Nivel de Anticuerpos
Si hay células inmunes cercanas, el nivel de anticuerpos aumenta gradualmente. Se toma el total de anticuerpos del vecindario, se divide entre 20, y ese valor se suma al nivel actual. El máximo es 100.
...
### Decaimiento Natural (Cada Turno)
El sistema tiene procesos de decaimiento natural para evitar valores permanentemente altos:
- **Carga viral**: Disminuye 2 puntos por turno (mínimo 0)
- **Nivel de anticuerpos**: Disminuye 1 punto por turno (mínimo 0)
- **Memoria inmune**: Disminuye 1 punto por turno (mínimo 0)
- **Nivel de energía**: Se regenera 1 punto por turno (máximo 100)
...
**Temperatura base:** 37.0°C (normal)

**Aumento por infección activa:** La tasa de infección (como porcentaje entre 0 y 100) se divide entre 100 y multiplica por 4, sumándose a la temperatura base. Una infección del 50% añade 2°C.

**Aumento por respuesta inmune (fiebre):** La cantidad de células inmunes activas se divide entre 100 y multiplica por 1.5. Esto representa la fiebre generada por la actividad inmunológica.

**Caída en zombificación completa:** Si el organismo está zombificado, la temperatura cae a 35-36°C (cuerpo "muerto"), variando aleatoriamente en ese rango.

La temperatura se mantiene entre 35.0°C (mínimo) y 42.0°C (máximo de fiebre).
...
### Requisitos
El proyecto requiere Python 3 y la librería tkinter (generalmente incluida con Python).

### Ejecución
Ejecuta el archivo principal del simulador desde la terminal.

### Controles
- **▶ Iniciar**: Ejecuta la simulación en modo continuo
- **⏸ Pausar**: Detiene la simulación temporalmente
- **⏭ Paso**: Avanza un solo turno del autómata
- **🔄 Reiniciar**: Reinicia completamente el sistema a su estado inicial
- **💉 Inyectar Virus**: Introduce un foco de infección zombie en una posición aleatoria
